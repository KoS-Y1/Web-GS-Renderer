...
@group(0) @binding(1) var<storage, read_write> instanceCount: atomic<u32>;
...
fn computeMain() {
    let n = min(atomicLoad(&instanceCount), uniforms.maxInstanceCount);

    outArgBuffer[0] = (n + RADIX_BLOCK_SIZE - 1u) / RADIX_BLOCK_SIZE; // ceil(n / blockSize)
    outArgBuffer[1] = 1u;
    outArgBuffer[2] = 1u;
